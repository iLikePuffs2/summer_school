...
@Service
public class SignInAnalyseImpl implements OptionalAnalysisService {
        ...
        switch (freeInfo.getStudentVariableNum()) {
            ...
            case 1:
                ...
                for (int i = 0; i < schoolLevelArr.length; i++) {
                    ...
                    SignInMapper mapper1 = sqlSession.getMapper(SignInMapper.class);
                    Integer avg = mapper1.countGroupAvg(hotSpotId, idList);
                    if (avg == null) {
                        integerVertical[i] = 0;
                    } else {
                        integerVertical[i] = avg;
                    }
                }
...
//        List<Student> users = mapper.selectByDifferent(student);
        ...
        switch (freeInfo.getStudentVariableNum()) {
            ...
            case 1:
                ...
                for (int i = 0; i < schoolLevelArr.length; i++) {
                    ...
                    //此时得到了这个符合固定条件的第一个学校层次的学生群体的全都id
                    //就可以在签到表里查出这个群体的积极性评分的均值了,然后存入纵坐标的数组即可
                    TopicMapper mapper1 = sqlSession.getMapper(TopicMapper.class);
                    Integer avg = mapper1.countTopicSignInAvg(topicId, idList);
                    if (avg == null) {
                        integerVertical[i] = 0;
                    } else {
                        integerVertical[i] = avg;
     ...
     */
    @Override
    public AnalyseBackDto solidDay(FreeStudentIntoDto solidInfo) {
        return null;
    }
...
//        List<Student> users = mapper.selectByDifferent(student);
        ...
        for (int i = 0; i < hotSpotList.size(); i++) {
            SignInMapper mapper1 = sqlSession.getMapper(SignInMapper.class);
            Integer value = mapper1.countGroupAvg(hotSpotList.get(i).getResearchHotSpotId(), idList);
            if (value == null) {
                integerVertical[i] = 0;
            } else {
                integerVertical[i] = value;
            }
...
//        List<Student> users = mapper.selectByDifferent(student);
        ...
        for (int i = 0; i < topicList.size(); i++) {
            TopicMapper mapper1 = sqlSession.getMapper(TopicMapper.class);
            Integer value = mapper1.countTopicSignInAvg(topicList.get(i).getId(), idList);
            if (value == null) {
                integerVertical[i] = 0;
            } else {
                integerVertical[i] = value;
