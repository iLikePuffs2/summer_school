...
@Service
public class ActiveAnalyseImpl implements OptionalAnalysisService {
    @Autowired
        ...
        switch (freeInfo.getStudentVariableNum()){
            ...
            case 1:
                ...
                for (int i = 0; i < schoolLevelArr.length; i++) {
                    ...
                    Integer avg = mapper1.countHotSpotActiveGroupAvg(hotSpotId, idList);
                    if (avg == null) {
                        integerVertical[i]=0;
                    }else{
        ...
        switch (freeInfo.getStudentVariableNum()){
            ...
            case 1:
                ...
                for (int i = 0; i < schoolLevelArr.length; i++) {
                    ...
                    Integer avg = mapper1.countTopicActiveGroupAvg(topicId, idList);
                    if (avg == null) {
                        integerVertical[i]=0;
                    }else{
                        integerVertical[i] = avg;
...
//        List<Student> users = mapper.selectByDifferent(student);
        ...
        for (int i = 0; i < hotSpotList.size(); i++) {
            HotSpotMapper mapper1 = sqlSession.getMapper(HotSpotMapper.class);
            Integer value = mapper1.countHotSpotActiveGroupAvg(hotSpotList.get(i).getResearchHotSpotId(), idList);
            if (value == null) {
                integerVertical[i] = 0;
            } else {
...
//        List<Student> users = mapper.selectByDifferent(student);
        ...
        for (int i = 0; i < topicList.size(); i++) {
            TopicMapper mapper1 = sqlSession.getMapper(TopicMapper.class);
            Integer value = mapper1.countTopicActiveGroupAvg(topicList.get(i).getId(), idList);
            if (value == null) {
                integerVertical[i] = 0;
            } else {
                integerVertical[i] = value;
