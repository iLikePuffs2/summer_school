...
@Service
public class CommitmentAnalyseImpl implements OptionalAnalysisService {
        ...
        switch (freeInfo.getStudentVariableNum()){
            ...
            case 1:
                ...
                for (int i = 0; i < schoolLevelArr.length; i++) {
                    ...
                    //就可以在研究热点参与表里查出这个群体的学习投入度的均值了,然后存入纵坐标的数组即可
                    HotSpotMapper mapper1 = sqlSession.getMapper(HotSpotMapper.class);
                    Integer avg = mapper1.countHotSpotCommitGroupAvg(hotSpotId, idList);
                    if (avg == null) {
                        integerVertical[i]=0;
                    }else{
        ...
        switch (freeInfo.getStudentVariableNum()){
            ...
            case 1:
                ...
                for (int i = 0; i < schoolLevelArr.length; i++) {
                    ...
                    Integer avg = mapper1.countTopicCommitGroupAvg(topicId, idList);
                    if (avg == null) {
                        integerVertical[i]=0;
                    }else{
                        integerVertical[i] = avg;
...
//        List<Student> users = mapper.selectByDifferent(student);
        ...
        for (int i = 0; i < hotSpotList.size(); i++) {
            HotSpotMapper mapper1 = sqlSession.getMapper(HotSpotMapper.class);
            Integer value = mapper1.countHotSpotCommitGroupAvg(hotSpotList.get(i).getResearchHotSpotId(), idList);
            if (value == null) {
                integerVertical[i] = 0;
            } else {
...
//        List<Student> users = mapper.selectByDifferent(student);
        ...
        for (int i = 0; i < topicList.size(); i++) {
            TopicMapper mapper1 = sqlSession.getMapper(TopicMapper.class);
            Integer value = mapper1.countTopicCommitGroupAvg(topicList.get(i).getId(), idList);
            if (value == null) {
                integerVertical[i] = 0;
            } else {
                integerVertical[i] = value;
